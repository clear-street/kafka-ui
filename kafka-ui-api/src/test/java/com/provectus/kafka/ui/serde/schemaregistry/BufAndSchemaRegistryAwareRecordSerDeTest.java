 ...
 * mock(BufSchemaRegistryClient.class);
 * 
 * private final BufAndSchemaRegistryAwareRecordSerDe serde = new
 * BufAndSchemaRegistryAwareRecordSerDe(
 ...
 * 
 * @Test
 * void callsSchemaFormatterWhenValueHasMagicByteAndValidSchemaId() throws
 * Exception {
 ...
 * bytesWithMagicByteAndSchemaId(schemaId, jsonToAvro(jsonValueForSchema,
 * schema))));
 * 
 ...
 * serde.protoValueSchemaFromHeaders(record.headers()).get();
 * assertThat(protoSchemaKey.getFullyQualifiedTypeName()).isEqualTo(
 * "protobuf_type.foo.v1.bar");
 * assertThat(protoSchemaValue.getFullyQualifiedTypeName()).isEqualTo(
 * "protobuf_type.dead.v1.beef");
 * }
 * 
 * @Test
 * void testProtoSchemaCorrectFromTopic() throws Exception {
 * ProtoSchema protoSchema =
 * serde.protoSchemaFromTopic("test-topic.proto.v1.foo.bar").get();
 * assertThat(protoSchema.getFullyQualifiedTypeName()).isEqualTo("v1.foo.bar");
 * }
 * 
 * private void assertJsonsEqual(String expected, String actual) throws
 ...
 * GenericDatumWriter<>(schema.rawSchema());
